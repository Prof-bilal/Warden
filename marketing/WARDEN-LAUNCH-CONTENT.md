...
7. Fail-closed: no usable backend ⇒ Warden refuses to run; the refusal error
   is tested. `VERIFIED`.
8. Distribution: GitHub Releases v0.1.3–v0.1.11, npm `warden-sandbox-cli`
...
| Security-conscious developers | `~/.ssh`, `.env`, AWS creds exposed to third-party code | Real credential loss | Deny-by-default + audit log | `testdata/proof/run-proof.sh` evidence, `docs/security.md` | High |
...
- Credential-threat tests are boundary tests, not attack-scenario audits:
  `~/.ssh/id_rsa` blocked (proof harness: `testdata/proof/run-proof.sh`), `/etc/shadow` probe denies in
