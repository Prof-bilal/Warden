...
**Image:** `warden-post-x-launch-1200x675.png` — Dark bg, orange shield left, white headline `YOUR MCP SERVERS CAN READ EVERYTHING` (Inter Black, 36px), muted subtext `MCP has no concept of a boundary.` Alt text: `Dark card with orange shield icon and headline about MCP servers having full system access`
...
**Image:** `warden-policy-example-800x600.png` — Code card: `policy.yaml` with filesystem/network/env/limits blocks highlighted in orange as named. Background `#1a1a1a`, border `#27272a`, JetBrains Mono.
