...
**Image:** `warden-post-facebook-launch-1200x630.png` — Dark `#0a0a0a` card, orange shield top (80×80), white headline `Run MCP servers safely` (Inter Black, 32px, centered), muted subtext `One YAML policy. OS-level enforcement. Deny by default.` (Inter Regular, 16px, `#a1a1aa`), code snippet `policy.yaml` faint bg at 10% opacity.  
**Alt text:** `Dark card with orange shield icon, headline about running MCP servers safely, and policy file code in background`
...
**Image:** `warden-terminal-run-1200x630.png` — Terminal: `warden run --policy policy.yaml -- node server.js` with green `✓ Sandbox active`. Dark terminal, JetBrains Mono.  
