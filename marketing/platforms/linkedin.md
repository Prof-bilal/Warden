...
**Image:** `warden-post-linkedin-launch-1200x627.png` — Dark `#10141A` card, blue shield top-left (80×80), white headline `The boundary MCP never had` (Geist Sans Bold, 32px), muted subtext `Sandbox runtime for MCP servers — open source, pre-1.0` (Geist Sans Regular, 16px, `#8D95A5`), code snippet `policy.yaml` faint in background at 10% opacity.  
**Alt text:** `Dark card with blue shield icon, headline about MCP sandbox, and policy file code in background`
...
**Image:** `warden-post-linkedin-network-1200x627.png` — Diagram: server box → blue proxy box → `✓ api.github.com` (green) / `✗ evil.com` (red, label `never resolved`). Dark `#10141A` bg, blue `#6E93E8` proxy, white labels.  
...
**Image:** `warden-post-linkedin-short-1200x627.png` — Minimal: dark bg, large white text `Your MCP servers don't need your whole filesystem.` (Geist Sans Bold, 36px, centered), small blue shield below, `github.com/Prof-bilal/Warden` muted at bottom.
