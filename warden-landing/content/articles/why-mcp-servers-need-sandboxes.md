...
For first-party tools that trust boundary is fine. For third-party MCP servers pulled from a registry, it is not. The server author decides what the process does, and the AI client decides what it asks the server to do — but nothing on your side constrains either of them.
...
Warden closes this gap by running MCP servers inside an OS-native sandbox with **deny-by-default** grants. Nothing is accessible — files, network hosts, environment variables — unless the policy explicitly allows it.
...
Most sandboxing tooling starts from "allow everything, then restrict." Warden inverts that: a server starts with no capabilities at all. If the policy file doesn't grant a path, a host, or a variable, the attempt is denied — and the denial is recorded.
...
Warden is open source — see the [quickstart](/docs/quickstart) to try it, or read [how Warden enforces policies](/blog/how-warden-enforces-policies) for the mechanics.
