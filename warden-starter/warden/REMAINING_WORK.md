...
## P0Fix the production bug exposed by the Windows job ✅ FIXED (`eadca83`)
...
**Why this matters:** same class of bug as the ETW onea DLL binding the
author assumed would be present but isn't on the actual hostexcept it
...
## P2Fix cross-platform test portability debt (Latentonly surfaced because CI ran)

Several tests were written assuming POSIX paths and a Linux $PATH, so they
failed on the first Windows run. These aren't backend bugsthe backends
...
**Acceptance:** the Windows job goes green on these packages' testswithout
...
## P3Loose ends from release work (minor)

These are things left over from moving the release pipeline to root and
shipping v0.1.3–v0.1.5.

**3.1Subproject `ci.yml` still lives at `warden-starter/warden/.github/`**
...
**3.2npm publish isn't rerun-safe (cosmetic, but real)**
...
so nothing was lostbut if a release job job fails late and is rerun, the
...
valuebut the repo's copy stays stale at `0.1.3`. Cosmetic, but it's the
...
**3.4v0.1.4's release run shows ❌ on the rerun (cosmetic)**
...
**3.5Homebrew tap not actually published (manual step)**
...
**3.6Confirm the second P0 (fwpuclnt.dll / FwpmEngineOpen) is fixed on the
